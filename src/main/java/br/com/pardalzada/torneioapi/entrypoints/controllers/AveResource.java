...
public class AveResource {
	...
	@ApiOperation(value = "Aceitar transferencia de ave")
	@PutMapping("/aceitar-transferencia/{id}")
	public ResponseEntity<AveHttp> updateAceitarTransferencia(@PathVariable Long id, @Valid @RequestBody AveHttp aveHttp) {
		AveEntity aveEntityConferenciaStatus = aveFindByIdUseCase.findById(id);
		if(aveEntityConferenciaStatus.getStatusTransferencia().toUpperCase().equals("A")) {
			throw new ResourceNotFoundException(aveEntityConferenciaStatus);
		}
		aveHttp.setStatusTransferencia("A");
		AveEntity aveEntity = aveUpdateUseCase.update(id, aveHttpMapper.to(aveHttp));
		log.info("Ave com o ID {} atualizada com sucesso", aveEntity.getCodigo());
		return ResponseEntity.status(HttpStatus.OK).body(aveHttpMapper.from(aveEntity));
	}
	
	@ApiOperation(value = "Recusar transferencia de ave")
	@PutMapping("/recusar-transferencia/{id}")
	public ResponseEntity<AveHttp> updateRecusarTransferencia(@PathVariable Long id, @Valid @RequestBody AveHttp aveHttp) {
		AveEntity aveEntityConferenciaStatus = aveFindByIdUseCase.findById(id);
		if(aveEntityConferenciaStatus.getStatusTransferencia().toUpperCase().equals("A")) {
			throw new ResourceNotFoundException(aveEntityConferenciaStatus);
		}
